...
render() {
  if [[ ! -f "$JSONL_PATH" ]]; then
    printf '\033[0;31mNo autotune session found.\033[0m\n'
    echo "Run: autotune start"
    return
  fi

  python3 - "$JSONL_PATH" "$STATE_PATH" "$FULL" <<'PYDASH' 2>/dev/null
import json
import os
import sys
...
for r in display_results:
    ...
    status_color = {"keep": GREEN, "discard": YELLOW, "crash": RED, "checks_failed": RED}.get(status, "")
    status_str = f"{status_color}{status}{RESET}" if status_color else status
